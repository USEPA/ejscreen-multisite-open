...
#' @export
#' 
doaggregate_old <- function(sites2blocks, sites2states_or_latlon=NA, countcols=NULL, popmeancols=NULL, calculatedcols=NULL, testing=FALSE, include_ejindexes=FALSE, updateProgress = NULL, need_proximityscore=FALSE, ...) {
  ...
  results_bysite_popmeans <- sites2bgs_plusblockgroupdata_bysite[   ,  lapply(.SD, FUN = function(x) {
    stats::weighted.mean(x, w = bgwt * pop, na.rm = TRUE)    # 100 msec
  ...
  results_bysite <- cbind(results_bysite, state.avg.cols_bysite)  # cbind??  faster way?  ************
