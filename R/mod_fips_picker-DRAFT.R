    ...
    sidebarLayout(
      sidebarPanel(
        ...
        selectInput("type2analyze", multiple = FALSE,
                    ...
                    selected = "Cities or Places"),
                    # selected = "Select Regions, States, Counties, or Cities"),
        
        checkboxGroupInput("regionpicker2", label = "EPA Region", 
                           choices = sort(regiontable$REGION), # sort(unique(placetable$eparegion)), 
                           selected = sort(regiontable$REGION), # sort(unique(placetable$eparegion)), 
                           inline = TRUE),
