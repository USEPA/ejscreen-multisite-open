#' ST_by_site_from_sites2blocks - Get State that each site is in, from a table of ejam_uniq_id, blockid, distance
...
ST_by_site_from_sites2blocks <- function(sites2blocks) {
  ...
  if (!all(c('ejam_uniq_id', 'blockid', 'distance') %in% names(sites2blocks) )) {
      stop("column names must include ejam_uniq_id, blockid, and distance, as in output of getblocksnearby() - see ?testoutput_getblocksnearby_10pts_1miles")}
