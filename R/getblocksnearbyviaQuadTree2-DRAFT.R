#' Find nearby blocks using Quad Tree data structure for speed, NO PARALLEL PROCESSING - DRAFT / WORK IN PROGRESS
#'       
...
getblocksnearbyviaQuadTree2 <- function(sitepoints, radius = 3, maxradius = 31.07, avoidorphans = FALSE, 
  ...
  for (i in 1:nRowsDf) {
    ...
    # could drop extra fields in f2 and only keep    #   .(siteid, FAC_X, FAC_Y, FAC_Z, x_low, x_hi, z_low, z_hi)
    ...
    res[[i]]     <- tmp[distance <= truedistance, .(blockid, distance, siteid)]  # ** SLOW STEP TO OPTIMIZE - how do you drop rows that meet some condition, just by reference??
