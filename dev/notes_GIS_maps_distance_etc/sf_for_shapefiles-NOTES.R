...
# also see functions defined in   /EJAM/R/map_blockgroups.R
...
## mapview pkg  mapview() to see tracts ####
usa
# library(# mapview)
mapview(usa[usa$ST == "DE", c("GEOID10", "SF", "CF", "DF_PFS", "FIPS.ST", "ST", "geometry") ])
mapview(usa[usa$ST == "NY", ])
usa_cejst_shape <- usa
rm(usa)
########################################################################### #
...
## _mapview to map it ####
## would need the mapview package here
mapview(ring, map.types = c("OpenStreetMap", "OpenTopoMap", "Esri.WorldImagery", "CartoDB.Positron"))
