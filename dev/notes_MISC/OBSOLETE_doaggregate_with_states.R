...
#'
doaggregate_with_states <- function(sites2blocks, countcols=NULL, popmeancols=NULL, calculatedcols=NULL, testing=FALSE, updateProgress = NULL, ...) {
  ...
  if(is.function(updateProgress)){
    ...
    boldtext <- paste0('Joining datasets')
    #smalltext <- paste0("Approx. ", timeleft, " to go.")
    updateProgress(message_main = boldtext, 
                   #message_detail = smalltext, 
                   value = 0.75)
  ...
  #incorporated the state abbreviations from EJSCREEN download
  STATE_Ab <- EJAMejscreendata::EJSCREEN_Full_with_AS_CNMI_GU_VI[,c("ID","ST_ABBREV")]
  ...
  sites2bgs_plusblockgroupdate_ST_s  <- merge(sites2bgs_bysite,
                                               state_blfips,
                                              all.x = TRUE, all.y=FALSE, by='bgid')

  sites2bgs_plusblockgroupdate_ST_o  <- merge(sites2bgs_overall,
                                               state_blfips,
                                              all.x = TRUE, all.y=FALSE, by='bgid')

  sites2bgs_plusblockgroupdate_bysite  <- merge(sites2bgs_plusblockgroupdate_ST_s,
                                                blockgroupstats[ , c('bgid', ..countcols, ..popmeancols, ..calculatedcols)],
                                                all.x = TRUE, all.y=FALSE, by='bgid')
  sites2bgs_plusblockgroupdate_overall <- merge(sites2bgs_plusblockgroupdate_ST_o,
                                                blockgroupstats[ , c('bgid', ..countcols, ..popmeancols, ..calculatedcols)],
                                                all.x = TRUE, all.y=FALSE, by='bgid')
  ...
  st <- unique(sites2bgs_plusblockgroupdate_bysite[ , c("siteid","ST")])

  results_bysite <- merge(results_bysite,
                          st,
                          all.x = TRUE, all.y=FALSE, by='siteid')
