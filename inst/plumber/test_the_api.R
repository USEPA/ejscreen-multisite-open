...
if (1 == 0) {
  ...
  # IT MUST BE PUT IN /inst/plumber/  of SOURCE package, 
  # since installing deletes anything like EJAM/plumber/ and moves /inst/whatever  to the root of installed version!
  # but while testing it is confusing since the location of the plumber.R file differs from where it will be in the installed package.
  # This would refer to where it will be once installed:
  # fname_installed <- system.file("plumber/plumber", package = "EJAM") # what was in inst folder of source pkg, installed version puts in root folder
  # This would refer to where it is during testing:
  #  fname_source <- file.path(mysource(), "/inst/plumber/plumber.R" )
  ...
  # If you’re running this code locally on your personal machine, 
    # you should be able to use the API in a browser
    
    # but not do these from the same R session I guess:
  ...
  browseURL("http://localhost:8000/echo")
  
  ##############  # 
  # try ejamit
  
  #  http://127.0.0.1:7705/ejamit?lat=31.34653&lon=-92.40151&radius=1.2&attachment=false&test=false 
  ...
  url2csv = "http://127.0.0.1:8036/ejamit_csv?lon=-101&lat=36&radius=1"
  browseURL(url2csv)
