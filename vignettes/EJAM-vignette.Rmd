...
industryword <- "pulp"
mysites <- EJAMfrsdata::frs[EJAMfrsdata::frs$REGISTRY_ID %in% unlist(
...
sitepoints639 <- EJAMfrsdata::frs[EJAMfrsdata::frs$REGISTRY_ID %in% unlist(
  EJAMfrsdata::get_siteid_from_naics(
    EJAM::naics_find(industryword, add_children = TRUE))[,"REGISTRY_ID"]),  ]
EJAMejscreenapi::mapfast(sitepoints639)
nrow(sitepoints639)
```
...
sitepoints2 <- latlon_readclean(testjunk)
```
...
# See map of how each site is surrounded by block points
...
# Demog.Index  versus  VSI.eo  names differ currently
# lat lon kept in API outputs.
# links to report or map
# statename, ST, REGION, etc. not yet in EJAM output.
# us and state averages not yet there.
# 
