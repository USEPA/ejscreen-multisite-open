...
```{r dataload_from_aws-indexblocks, eval = TRUE}
library(EJAM)
...
### View Results as Spreadsheet (Launches Excel)

```{r table_xls_from_ejam-launchexcel, eval = FALSE}
table_xls_from_ejam(ejamit(testpoints_10, radius = 1), launchexcel = T, save_now = F)
```

### Save Results as Spreadsheet

```{r table_xls_from_ejam-savenow, eval = FALSE}
table_xls_from_ejam(ejamit(testpoints_10, radius = 1), save_now = T)
```
...
### >>>>MORE ABOUT SELECTING SITES ETC. ####
...
```{r plotblocksnearby_1, fig.height=5, fig.width=5}
plotblocksnearby(testpoints_n(1), radius = 3, returnmap = TRUE)
```
