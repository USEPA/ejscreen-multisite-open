...
```{r}
# Quick basic map of one industrial category, with popup for each site
# (but note that FRS lacks NAICS for most facilities!)
# 
mapfast(mysites <- frs_from_naics("smelt"))
...
mysites <- cbind(`ECHO report` = link1, 
                 ...
                 mysites)
DT::datatable(
  mysites[1:min(nrow(mysites), 2000) ], # >2k rows is too much for client-side DataTables
  escape = FALSE,
...
# search for industry codes or names
