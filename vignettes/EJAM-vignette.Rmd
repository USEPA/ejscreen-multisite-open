...
EJAM is not only a web app built in shiny R, it is also an R package, using the [golem](https://thinkr-open.github.io/golem/ "https://thinkr-open.github.io/golem/") framework.

### To install the EJAM R package and related packages

To install the latest development version of EJAM, you will first need to make sure you have access to the GitHub repositories on EPA's enterprise github site, at [USEPA GitHub](https://github.com/USEPA "https://github.com/USEPA"). You need to use VPN or be inside the EPA LAN at the office.
...
Try to install the packages using `devtools::install_github()`
...
Once the packages are installed, to use EJAM in the console you would start like this:
  ...
  shiny::fluidPage(
    tabsetPanel(
      tabPanel(title = "api app",
               ...
               EJAM:::mod_ejscreenapi_ui(
                 "TESTID", simpleradius_default_for_ui = 2
               )
      ),
      tabPanel(title = "results in overall app",
               DT::dataTableOutput("results")
      ),
      tabPanel("info", renderText("testinfo2"))
    )
  )
}
######################### #
TEST_SERVER <- function(input, output, session) {
  ...
  output$testinfo2 <- renderText(cat(
    "x names:  ", paste0(names(x()), collapse = ", "), "\n"
    ))
  
}
######################### #
shinyApp(ui = TEST_UI, server = TEST_SERVER) # Test module, as mini-app
