...
```{r eval = FALSE}
dataload_from_aws() # do this once, then save data locally so future sessions start faster
...
```{r mapfast(frs_from_naics), eval = TRUE, fig.height=5, fig.width=5}
  if (!exists("frs_from_naics")) dataload_from_pins("frs_from_naics")
...
-   **run_app()** to launch the web app locally
    - A couple of functions help load data files, like dataload_from_local(), dataload_from_aws(), etc.
