...
cbind(path = x <- list.dirs(
  system.file("testdata/", package = "EJAM"),
  recursive = FALSE),
  folder = basename(x))
```
...
for (example_type in x) {
  ...
  print(cbind(`Examples Available` = list.files(example_type)))
  }
```
...
Note the largest files included in the package are mostly the blockgroupstats
