...
To open the locally installed "testdata" folders (in Windows File Explorer, or iOS Finder)
...
for testing ejamit(), mapfast(), ejscreenit(), getblocksnearby(), etc.
...
**EXAMPLES OF OUTPUTS**
from ejamit(), ejscreenit(), getblocksnearby(), etc.,
you can use as inputs to ejam2report(), ejam2excel(), ejam2ratios(), ejam2barplot(), doaggregate(), etc.
```{r testout_data, eval=TRUE, echo=TRUE}
x[grepl("^testout", x$Item), ]
cat("\n\n")
```

Note that the largest files included in the package are mostly the blockgroupstats
dataset from EJScreen. Note the block datasets are not in the package - they get
downloaded by the package at launch or as needed.
...
```{r barplot, eval=TRUE, echo=TRUE, fig.height=5, fig.width=7}
ejam2barplot(out)
```
...
```{r onesiteyy, eval=TRUE, echo=TRUE}
ejam2barplot(out1)
```
...
Plot that, comparing sites, but for just one demographic indicator

```{r barplot_compare_sites_one_D, eval=TRUE, echo=TRUE}
...
```{r ejamit_compare_distances2plot, eval=TRUE, echo=TRUE, fig.height=5, fig.width=7}
...
```{r plot_distance_by_pctd, eval=TRUE, echo=TRUE}
...
```{r plot_distance_by_pctd2, eval=TRUE, echo=TRUE}
...
for (i in 1:NROW(pts)) {
  plot_distance_by_pctd(s2b, sitenum = i, score_colname = "pctlowinc")
...
Out of all the residents within the area analyzed, see how some are mostly nearby
and others are further away, as a CDF plot.
...
```{r distance_by_group_by_site, eval=TRUE, echo=TRUE}
...
```{r diagnostics, echo=TRUE, eval=TRUE}
...
```{r plotblocksnearby2, eval=TRUE, echo=TRUE, fig.height=5, fig.width=5}
...
```{r plot_count_of_blocks_nearby, eval=TRUE, echo=FALSE, fig.height=5, fig.width=6}
...
```{r quantiles, eval=TRUE, echo=TRUE}
quantile(s2b_stats$blocks_within_1mile, probs = (0:4) * 0.25)
```

### Map sites with popups about how many blocks were found near each site

```{r s2b_stats, eval=TRUE, echo=TRUE, fig.height=5, fig.width=5}
...
```{r, doplotblocksnotshowoutput, eval=TRUE, echo=FALSE}
