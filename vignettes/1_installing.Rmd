...
remotes::install_github('USEPA/EJAM',
      dependencies=T, build=F, build_vignettes=F, build_manual=F, upgrade=F)
```

To install the development version, which may have more features but not yet be fully tested:

```{r install_github, eval=FALSE, include=TRUE}
if (!require(remotes)) {install.packages("remotes")}
...
If installing from a public repository, and you use R but not for package development, so you do not already have git installed and do not have a personal access token set up, then you might have to use `auth_token = ""` as another parameter within `install_github()`.

------------------------------------------------------------------------

### if you need the full source code or want to build/install in RStudio on your own

The EJAM package is available as one of the [USEPA github](https://github.com/USEPA/EJAM "github.com/USEPA/EJAM"){.uri target="_blank" rel="noreferrer noopener"} repositories (not on CRAN).
...
Note that if a USEPA github repository is not public, you need to be inside EPA's network (e.g., via VPN) and also need to get a personal access token (PAT):
