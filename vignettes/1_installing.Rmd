...
To install EJAM, you will need to have access to the relevant GitHub repositories on EPA's enterprise github site, at [USEPA GitHub](https://github.com/USEPA "https://github.com/USEPA"){.uri target="_blank" rel="noreferrer noopener"}.

## How to install 

### if you do not need to work with the source code

If you just want to install EJAM but not work with source code, you can try this approach that uses `remotes::install_github()`:
...
There are 3 EJAM-specific packages as of early 2024 that EJAM needs, all of which are in EPA github repositories:
...
Note that if a USEPA github repository is not public, you need to be inside EPA's network (e.g., via VPN) and may also need to get a personal access token (PAT):
...
3.  Try again to install the packages using `devtools::install_github()`
