...
test_that("getblocks_diagnostics does not crash", {
  expect_no_error({
    sink(tempfile())
    on.exit(sink(NULL))
    getblocks_diagnostics(testoutput_getblocksnearby_1000pts_1miles, detailed = FALSE,  see_pctiles = FALSE) 
    getblocks_diagnostics(testoutput_getblocksnearby_1000pts_1miles, detailed = TRUE,  see_pctiles = TRUE) 
    getblocks_diagnostics(testoutput_getblocksnearby_1000pts_1miles, detailed = TRUE,  see_pctiles = FALSE ) 
    getblocks_diagnostics(testoutput_getblocksnearby_1000pts_1miles, detailed = FALSE,  see_pctiles = TRUE) 
  } )
})
