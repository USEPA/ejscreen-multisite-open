...
# in sic_from_any.R
# library data.table
# not the best name since it returns a subset of frs
...
test_that('warning and error for invalid SIC',{
  expect_warning(expect_error(val <- latlon_from_sic("glass")))
  expect_warning(expect_error(val <- latlon_from_sic("blue")))
})
