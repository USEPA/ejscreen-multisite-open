## unit tests for EJAM::fipsbg_from_anyfips
...
test_that('by state',{
  expect_no_warning(val <- fipsbg_from_anyfips(36))
  expect_no_warning(val <- fipsbg_from_anyfips("36"))
...
test_that('by  tract',{
  expect_no_warning(val <- fipsbg_from_anyfips(36071000100))
  expect_no_warning(val <- fipsbg_from_anyfips("36071000100"))
...
test_that('by  block group',{
  expect_no_warning(val <- fipsbg_from_anyfips(360710001001))
  expect_no_warning(val <- fipsbg_from_anyfips("360710001001"))
...
test_that('by  block ',{
  expect_no_warning(val <- fipsbg_from_anyfips(c(360710108011012, 360710108011006, 360710108011023)))
  expect_no_warning(val <- fipsbg_from_anyfips(c("360710108011012", "360710108011006", "360710108011023")))
  expect_equal(length(val), 1)
...
test_that('test leading zero addition',{
  expect_no_warning(val <- fipsbg_from_anyfips("1055")) # county
  expect_no_warning(val <- fipsbg_from_anyfips(1055))
  expect_equal(length(val), 90)
  expect_equal(substr(val[1], 1,2) , "01")

  expect_no_warning(val <- fipsbg_from_anyfips("1")) # state
  expect_no_warning(val <- fipsbg_from_anyfips(1))
  expect_equal(length(val), 3925)
  expect_equal(substr(val[1], 1,2) , "01")
  
  expect_no_warning(val <- fipsbg_from_anyfips("1055011002")) # tract
  expect_no_warning(val <- fipsbg_from_anyfips(1055011002))
 ...
 # removed block group level since it assumes it is a census tract code 
  ...
  expect_no_warning(val <- fipsbg_from_anyfips(10690401001010)) # bg
  expect_no_warning(val <- fipsbg_from_anyfips("10690401001010"))
  expect_equal(length(val), 1) # looking for "010005051900" and got 3 values like "010005051900"
  expect_equal(substr(val[1], 1,2) , "01")
  
})

testthat('leading zero addition fails for census block group level', {
  
  # cant handle leading zero at bg level since it assumes it's a census tract code
  expect_no_warning(val <- fipsbg_from_anyfips("10005051900")) # bg
  expect_no_warning(val <- fipsbg_from_anyfips("10005051900"))
  expect_equal(length(val), 1) # looking for "010005051900" and got 3 values like "010005051900"
  expect_equal(substr(val[1], 1,2) , "01")
})

# no warnings for invalid strings, no string cleaning (dashes/dots not removed)
testthat('invalid text strings', {
  expect_warning(val <- fipsbg_from_anyfips("blue"))
  expect_warning(val <- fipsbg_from_anyfips("36-071"))
  expect_warning(val <- fipsbg_from_anyfips("36-07"))
  expect_warning(val <- fipsbg_from_anyfips("$1001"))
