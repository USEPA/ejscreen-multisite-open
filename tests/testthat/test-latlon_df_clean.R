...
test_that('warns if no alias found, returns the same data frame',{
  expect_warning(expect_warning(expect_warning(val <- latlon_df_clean(data.frame('trilat' = 1, 'belong' = 2, 'belong' = 3 ))) ))
  expect_equal(colnames(val), c('trilat' ,'belong','belong'))
})
...
test_that('only the best alias #2', {
  expect_warning( expect_warning(val <- latlon_df_clean(data.frame("a" = "-100.75%", "LONGITUDE" = 200, "Long" = " 175 ", "lat" = "$35"))))
...
test_that('case variants left alone',{
  ...
  expect_warning(val <- latlon_df_clean(data.frame("a" = "-100.75%", "longing" = 200, "Lat" = " 175 ", "lat" = 35, "Lon" = "-66.4")))
  # lat lon can be cleaned
  expect_warning(val <- latlon_df_clean(data.frame("a" = "-100.75%", "longing" = 200, "Lat" = " 175 ", "lat" = "$35", "Lon" = "-66.4")))
  
  expect_warning( # infer col names
